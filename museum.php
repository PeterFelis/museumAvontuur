 ...
 */
 ...
 //inladen php file voor de component
...
?>
    ...
    <p>
        <label for="museum_text_color">Textkleur (hexcode):</label>
        <input type="text" id="museum_text_color" name="museum_text_color" pattern="^#([A-Fa-f0-9]{6}|[A-Fa-f0-9]{3})$" value="<?php echo esc_attr(get_post_meta($post->ID, 'museum_text_color', true)); ?>" placeholder="#ffffff" />
    </p>
    <p>
...
function museum_reviews_save_metabox($post_id)
{
    ...
    update_post_meta($post_id, 'museum_text_color', sanitize_hex_color($_POST['museum_text_color']));
}
